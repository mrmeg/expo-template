...
so the `html[data-theme=…]` CSS paints the right body background on byte 1. It
also stamps `data-ssr-system-scheme` with the system scheme the server used:
the client hint is a *request* header that browser JS cannot read, so without
that attribute the client's first render would have to guess with
`window.matchMedia` and would disagree with the server on every hint-less
request.

The blocking `COLOR_SCHEME_SCRIPT` stays as a **first-visit-only** failsafe and
now returns early when `data-theme` is already present — i.e. it's a no-op for
anyone who has ever set a preference. It still covers the one case no cookie can:
a brand-new visitor whose OS is dark and whose browser sent no hint. Don't widen
its 500ms `theme-loading` window; the cookie is what fixes the common case.
...
curl -s localhost:3000/showcase | grep -o 'data-theme="[^"]*"'   # data-theme="light"
```
