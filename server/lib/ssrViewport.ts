...
//
// Three read surfaces, mirroring `ssrOnboarding.ts`:
//
...
//      layouts cannot export loaders (`@expo/router-server`'s server manifest
//      only carries leaf html routes). Wiring it here covers every SSR route
//      at once instead of repeating a wrapper on all 31 leaf routes.
//
...
//
//   3. `detectSsrViewportWidth(request)` / `withSsrViewport(loader)` — the
//      explicit loader-friendly forms, kept for routes that also want the
//      width in loader data (pair with `<SsrViewportProvider>`):
//
...
//        }
//
//      These are now optional — the root wiring already seeds every route.
...
// desktop pair reproduces SSR_VIEWPORT_DEFAULT_HEIGHT (1280 → 800) so this
// agrees with @mrmeg/expo-ui's documented default.
 ...
 * the same per-request leak the theme store has to avoid.
 */
