...
- Web is client-rendered: `web.output: "server"` runs API routes, middleware, and data loaders, but routes are not server-rendered per request. Verify web changes in a browser against `bun run build && bun run start`, not only Jest or `tsc`.
