 ...
 *
 * The fix: resolve the value from a signal *both* sides have, so the server
 * HTML and the browser's first render agree.
 *
...
import { DEFAULT_VIEWPORT_WIDTH } from "@mrmeg/expo-ui/hooks";
...
export function resolveSsrViewportWidthForRender(): number {
  if (Platform.OS !== "web") return DEFAULT_VIEWPORT_WIDTH;
