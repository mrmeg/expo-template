...
describe("InputOTP", () => {
  it("renders correct number of cells for default length (6)", async () => {
  ...
  it("renders correct number of cells for custom length", async () => {
    await render(<InputOTP value="" onChangeText={() => {}} length={4} />);
  ...
  it("auto-focuses when explicitly enabled", async () => {
    await render(<InputOTP value="" onChangeText={() => {}} autoFocus />);

    const label = screen.getByLabelText("Verification code input");
    expect(label.props.autoFocus).toBe(true);
  });

  it("renders cells with position accessibility labels", async () => {
    await render(<InputOTP value="" onChangeText={() => {}} length={4} />);
  ...
  it("displays entered characters", async () => {
    await render(<InputOTP value="12" onChangeText={() => {}} length={4} />);
  ...
  it("renders error state", async () => {
    await render(<InputOTP value="" onChangeText={() => {}} error errorText="Invalid code" />);

    expect(screen.getByText("Invalid code")).toBeTruthy();
  });

  it("calls onComplete when all cells are filled", async () => {
