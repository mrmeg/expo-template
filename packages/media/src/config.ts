...
export interface MediaConfig<
  TBuckets extends Record<string, MediaBucketConfig> = Record<string, MediaBucketConfig>,
  TMediaTypes extends Record<string, MediaTypeConfig<Extract<keyof TBuckets, string>>> = Record<
    string,
    MediaTypeConfig<Extract<keyof TBuckets, string>>
  >,
> {
  buckets: TBuckets;
...
export type MediaTypeName<TConfig extends MediaConfig> = Extract<
  keyof TConfig["mediaTypes"],
  string
>;
...
export const DEFAULT_CONTENT_TYPE_EXTENSIONS: Record<string, string> = {
  "image/jpeg": "jpg",
  "image/jpg": "jpg",
  "image/png": "png",
  "image/webp": "webp",
...
export function createMediaConfig<const TConfig extends MediaConfig>(
  config: TConfig,
): TConfig {
  return config;
}
...
export function resolveContentTypeExtension(contentType: string): string | null {
  const normalized = contentType.toLowerCase().split(";")[0]?.trim() ?? "";
  return DEFAULT_CONTENT_TYPE_EXTENSIONS[normalized] ?? null;
}

export function getMediaTypeNames(config: MediaConfig): string[] {
  return Object.keys(config.mediaTypes);
}

export function validateMediaConfig(config: MediaConfig): MediaConfigValidation {
  ...
  for (const [bucketName, bucket] of Object.entries(config.buckets)) {
    if (isBlank(bucket.bucket)) missing.push(`buckets.${bucketName}.bucket`);
    if (isBlank(bucket.region)) missing.push(`buckets.${bucketName}.region`);
...
export function isAllowedContentType(
  ...
  const normalized = contentType.toLowerCase().split(";")[0]?.trim() ?? "";
  return settings.allowedContentTypes.some(
