 ...
 */
export function canvasLongEdgeLimitFor(userAgent: string | undefined | null): number {
  if (!userAgent) return CANVAS_LIMIT_DEFAULT;
  ...
  if (/iphone|ipad|ipod/.test(ua)) return CANVAS_LIMIT_IOS;
  if (/crios|edgios|fxios/.test(ua)) return CANVAS_LIMIT_IOS;
  if (/firefox|fxios/.test(ua)) return CANVAS_LIMIT_FIREFOX;
  if (/chrome|chromium|edg\//.test(ua)) return CANVAS_LIMIT_CHROMIUM;
  if (/safari/.test(ua)) return CANVAS_LIMIT_CHROMIUM;

  return CANVAS_LIMIT_DEFAULT;
}

/** Reads the ambient UA when there is one. Never throws in a non-DOM context. */
export function currentCanvasLongEdgeLimit(): number {
  const ua =
 ...
 */
export function clampLongEdgeToCanvasLimit(longEdge: number, limit: number): number {
  if (!Number.isFinite(longEdge) || longEdge <= 0) return longEdge;
  if (!Number.isFinite(limit) || limit <= 0) return longEdge;
