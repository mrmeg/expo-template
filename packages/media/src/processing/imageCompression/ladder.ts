...
import { formatFileSize, longEdgeOf } from "./utils";
 ...
 */
export function resolveLadderRungs(
  rungs: readonly number[],
  sourceLongEdge: number,
): number[] {
  const usable = rungs.filter((rung) => Number.isFinite(rung) && rung > 0);
  if (usable.length === 0) return sourceLongEdge > 0 ? [sourceLongEdge] : [];
...
export async function runDimensionLadder(
  options: DimensionLadderOptions,
  ...
  for (let index = 0; index < rungs.length; index += 1) {
    ...
    try {
      ...
      if (best) dispose(best);
      throw error;
    }

    const attempt: CompressedImage = {
      ...encoded,
      rung,
      attempts: index + 1,
    ...
    if (!attempt.overBudget) {
      logDev(
        `Ladder fit at ${rung}px: ${formatFileSize(attempt.size)} (attempt ${attempt.attempts})`,
      );
      return attempt;
    }
    ...
    logDev(
      `Ladder ${rung}px produced ${formatFileSize(attempt.size)} > budget ${formatFileSize(byteBudget)}${
        isLastRung ? " — last rung, returning over budget" : ", stepping down"
      }`,
    );
  }
  ...
  return best as CompressedImage;
}
