...
function saveFormatFor(format: UploadOutputFormat): SaveFormat {
 ...
 */
export function statSize(uri: string): number {
...
export async function encodeImageNative(
...
export function disposeEncodedImageNative(image: { uri: string }): void {
  try {
...
async function measureNative(source: ImageSource): Promise<number> {
  return statSize(source.uri);
}

async function probeDimensionsNative(
  source: ImageSource,
 ...
 * evidence used here.
 */
async function sniffContentTypeNative(
  source: ImageSource,
  fileName?: string,
): Promise<string | null> {
