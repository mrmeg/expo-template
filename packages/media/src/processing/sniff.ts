...
import { isUnknownContentType, normalizeContentType } from "./uploadPolicy";
 ...
 */
export function sniffContentTypeFromBytes(bytes: Uint8Array | null | undefined): string | null {
 ...
 */
export function contentTypeFromFileName(fileName: string | null | undefined): string | null {
...
export function resolveSourceContentType(input: {
  ...
  return (
    sniffContentTypeFromBytes(input.bytes) ?? contentTypeFromFileName(input.fileName)
  );
}
