...
export async function compressImage(options) {
    ...
    logDev(`[Web] Compressing image: ${width}x${height} -> ${targetWidth}x${targetHeight}, quality: ${config.quality}, format: ${format}`);
    // Load image
    ...
    while (shouldContinueCompression(blob.size, config.maxSizeKB, quality, config.minQuality)) {
        quality = reduceQuality(quality);
        logDev(`Image still ${formatFileSize(blob.size)} > ${config.maxSizeKB}KB, reducing quality to ${quality}`);
