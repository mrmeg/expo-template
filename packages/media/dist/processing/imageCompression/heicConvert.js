...
import { JPEG_CONTENT_TYPE, isHeicContentType, isUnknownContentType, } from "../uploadPolicy.js";
...
export async function readLeadingBytes(blob, count = SNIFF_BYTE_COUNT) {
 ...
 */
export async function isHeicBlob(blob, fileName) {
    if (isHeicContentType(blob.type))
        return true;
    if (hasHeicExtension(fileName))
        return true;
    if (!isUnknownContentType(blob.type))
        return false;
    try {
 ...
 */
const lazyHeicDecoder = async (options) => {
 ...
 */
export async function convertHeicToJpeg(blob, fileName, decoder = lazyHeicDecoder) {
    logDev(`Converting HEIC to JPEG: ${formatFileSize(blob.size)}${fileName ? ` (${fileName})` : ""}`);
    let converted;
    try {
        ...
        throw new MediaProcessingError("heic-conversion-failed", "Could not read this HEIC photo.", { contentType: blob.type, cause: error });
    }
    // heic2any returns Blob[] for multi-image files (bursts, Live Photos).
    const result = Array.isArray(converted) ? converted[0] : converted;
    if (!(result instanceof Blob) || result.size === 0) {
        throw new MediaProcessingError("heic-conversion-failed", "HEIC conversion produced no image data.", { contentType: blob.type });
    }
 ...
 * upload policy.
 */
export async function convertHeicToJpegIfNeeded(blob, fileName, decoder = lazyHeicDecoder) {
