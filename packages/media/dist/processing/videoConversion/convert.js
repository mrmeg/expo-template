...
import { CONVERSION_PRESETS, DEFAULT_PRESET, FFMPEG_CDN_BASE_URL, FFMPEG_WORKER_URL, } from "./config.js";
import { getFormatFromMimeType, TARGET_MIME_TYPE } from "./utils.js";
// ============================================================================
...
async function toBlobURL(url, mimeType) {
    ...
    try {
        ...
        throw new FFmpegWorkerUnavailableError(url, undefined, error instanceof Error ? error.message : undefined);
    }
    if (!response.ok) {
    ...
    if (mimeType === "text/javascript" && contentType.includes("text/html")) {
        throw new FFmpegWorkerUnavailableError(url, response.status, `Expected JavaScript worker at ${url}, got ${contentType}`);
    }
 ...
 */
function createWorkerBlobURL(workerURL) {
 ...
 */
async function loadFFmpeg(onLoadingFFmpeg) {
    ...
    loadPromise = (async () => {
        try {
            ...
            await ffmpeg.load({
                ...
                classWorkerURL: await toBlobURL(`${window.location.origin}${FFMPEG_WORKER_URL}`, "text/javascript"),
            });
 ...
 * ```
 */
export async function convertVideo(uri, originalMimeType, options = {}) {
    ...
    try {
        ...
        logDev(`Conversion complete: ${originalFormat} -> MP4 (${(blob.size / 1024 / 1024).toFixed(2)}MB)`);
