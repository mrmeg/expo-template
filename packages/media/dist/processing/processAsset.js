...
function resolveKind(asset, contentType) {
 ...
 * the file name.
 */
async function identifyContentType(asset, source, adapter, notifyPhase) {
    ...
    const sniffed = normalizeContentType(await adapter.sniffContentType(source, asset.fileName ?? undefined));
    if (sniffed && !isUnknownContentType(sniffed))
        return sniffed;
    return normalizeContentType(contentTypeFromFileName(asset.fileName) ?? contentTypeFromFileName(asset.uri));
}
/** Source size, preferring the picker's number and never failing the asset. */
async function measureSource(asset, source, adapter) {
    if (typeof asset.size === "number" && asset.size > 0)
        return asset.size;
    if (asset.blob && asset.blob.size > 0)
        return asset.blob.size;
    try {
...
async function processImageAsset(options, adapter, contentType, notifyPhase) {
    const { asset, allowlist } = options;
    const applied = [];
    const original = { uri: asset.uri, blob: asset.blob };
    const policy = resolveUploadFormatPolicy({
        contentType,
        allowlist: allowlist.image,
    });
    if (policy.action === "reject") {
        throw new MediaProcessingError("unsupported-format", policy.reason ?? "This file type cannot be uploaded.", { contentType: policy.sourceContentType });
    }
    ...
    try {
        ...
        const format = config?.format ?? policy.outputFormat ?? "jpeg";
        ...
        if (choice.picked === "original") {
            ...
            logDev(`Kept original ${policy.sourceContentType} (${formatFileSize(originalSize)}); re-encode was ${formatFileSize(processed.size)}`);
        }
        else {
            applied.push(`resize:${processed.rung}`, `encode:${processed.contentType}`);
            if (processed.overBudget)
                applied.push("over-budget");
        }
        ...
        if (intermediate)
            adapter.dispose(intermediate);
    }
}
async function processVideoAsset(options, adapter, contentType, notifyPhase) {
    const { asset, allowlist } = options;
    const applied = [];
    const originalSize = await measureSource(asset, { uri: asset.uri, blob: asset.blob }, adapter);
    let uri = asset.uri;
    let blob = asset.blob;
    ...
    if (needsConversion(contentType) && originalSize <= MAX_CLIENT_CONVERSION_SIZE) {
        ...
        try {
            ...
            const converted = await convertVideo(temporaryUri ?? uri, contentType, {
                ...
                onLoadingFFmpeg: () => notifyPhase({ type: "converting-video", loadingConverter: true }),
            });
            const choice = chooseUploadCandidate({ uri, blob, contentType, size: originalSize }, {
                uri: converted.uri,
                blob: converted.blob,
            ...
            if (temporaryUri)
                URL.revokeObjectURL(temporaryUri);
        }
    }
    let thumbnail;
    try {
...
function assertAllowlisted(contentType, allowlist) {
    if (isAllowlistedContentType(contentType, allowlist))
        return;
    throw new MediaProcessingError("unsupported-format", `${contentType || "This file"} cannot be uploaded.`, { contentType: normalizeContentType(contentType) });
}
function freeze(upload) {
 ...
 */
export async function processAsset(options) {
    const adapter = options.adapter ?? imagePlatformAdapter;
    const { asset } = options;
    const notifyPhase = (phase) => options.onPhase?.(asset, phase);
    const contentType = await identifyContentType(asset, { uri: asset.uri, blob: asset.blob }, adapter, notifyPhase);
    ...
    logDev(`Processed ${result.kind}: ${formatFileSize(result.size)} ${result.contentType} (${result.width}x${result.height}) [${result.applied.join(", ")}]`);
    return result;
}
