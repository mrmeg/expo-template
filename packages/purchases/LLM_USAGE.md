...
neither live source has reported this session. A lifetime grant reads as
`until: null`; the server stores it as `LIFETIME_UNTIL`. Switching `userId`
rescopes the store synchronously (nothing from the previous user grants or is
persisted under the new id) and identity calls are serialised. A gate on another entitlement
