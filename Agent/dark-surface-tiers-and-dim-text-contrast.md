...
- `packages/ui/src/components/Drawer.tsx:641` and `:804` paint the rail/overlay panel with `theme.colors.background`, so chrome and content are currently the same tier.
...
4. **Contrast floor test** (new, e.g. `src/constants/__tests__/colors.test.ts`) using local WCAG luminance math, asserting for **both schemes**:
   ...
   - `borderStrong` differs from `muted` and `secondary` (guards against the current invisible-border regression), and surface tiers are strictly ordered by luminance: `surfaceSunken` < `background` < `card` in dark; reverse in light.
